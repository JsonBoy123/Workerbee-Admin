                                ...
                                <div class="d-flex flex-wrap align-items-center gap-3">
                                    <div class="form-group">
                                        <select id="state-dd" class="form-control">
                                            <option selected="" hidden="">Apartment</option>
                                        </select>    
                                    </div>
                                </div>
...
@push('script')
    <script>
        ...
        $('#city').on('change' ,function () {
            ...
            $.ajax({
                url: "{{route('admin.booking.city-name')}}",
                type: "POST",
                data: {
                    cityName: cityName,
                    _token: '{{csrf_token()}}'
                ...
                success: function (result) {
                    $('#state-dd').html('<option value="">Select Apartment</option>');
                    $.each(result.city, function (key, value) {
                        $("#state-dd").append('<option value="' + value
                            .id + '">' + value.apartment_name + '</option>');
                    });
         ...
         $('#state-dd').on('change' ,function () {
            var apartmentId = $(this).val();
             ...
             $.ajax({
                url: "{{url('admin.booking.apartment-info')}}",
                type: "POST",
                data: {
                ...
                success: function (result) {
                    $.each(result.tableData, function (key, value) {
                        $("#table-data").append('<td>'+ value.id+'</td><td>'+value.customer_id+'</td><td>'+value.total_tax_amount+'</td><td>'+value.customer_id+'</td><td>'+value.total_tax_amount+'</td><td>'+value.total_tax_amount+'</td><td>'+{route('/')}+'</td>');
                    });
                    $('#city-dd').html('<option value="">Select City</option>');
                }
            });
